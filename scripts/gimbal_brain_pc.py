...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--weights", default="models/epoch_149.pt", help="YOLOv7 .pt weights")
    ...
    parser.add_argument("--kp-pan", type=float, default=10.0, help="Proportional pan gain in steps/s per pixel")
    parser.add_argument("--kp-tilt", type=float, default=10.0, help="Proportional tilt gain in steps/s per pixel")
    parser.add_argument("--deadband", type=float, default=12.0, help="Pixel error deadband")
    parser.add_argument("--max-speed", type=float, default=4000.0, help="Absolute velocity clamp in steps/s")
    ...
    parser.add_argument("--track-thresh", type=float, default=0.65, help="Minimum score to create a new track")
    parser.add_argument("--track-match-iou", type=float, default=0.20, help="Minimum IoU to match an existing track")
    parser.add_argument(
        "--track-max-age",
        type=int,
        default=None,
        help="Frames to keep an unmatched track internally; defaults to 15, or --reid-memory-frames when Re-ID is enabled",
    )
    parser.add_argument("--track-min-hits", type=int, default=3, help="Consecutive hits before a track is exposed to UI/control")
    parser.add_argument("--enable-reid", action="store_true", help="Enable OSNet Re-ID for ID stability")
    parser.add_argument("--reid-model", default="osnet_x0_25", help="torchreid OSNet model name")
    parser.add_argument("--reid-weights", default=None, help="Optional custom Re-ID weights path")
    parser.add_argument("--reid-candidates", type=int, default=4, help="Top detections to embed per processed frame")
    ...
    parser.add_argument("--reid-memory-frames", type=int, default=90, help="Track feature memory in frames when Re-ID is enabled")
...
class BrainUi:
    ...
    def handle_click(self, x: int, y: int, tracks: list[dict]) -> Optional[tuple[str, Optional[int]]]:
        ...
        if self.video_rect.collidepoint(x, y):
            track_id = pick_track_at(tracks, (x - self.video_rect.x, y - self.video_rect.y))
    ...
    def render(
        ...
        if self._screen is None:
            return
        self.video_rect = pygame.Rect(0, 0, width, height)
        self._buttons.clear()
        ...
        self._render_panel(width, height, tracks, state, error, fps, decoded)
        pygame.display.flip()
        self._clock.tick(60)

    def render_status(self, state: UiState, message: str) -> None:
        width, height = self.video_rect.size
        ...
        self._render_panel(width, height, [], state, None, 0.0, None)
        pygame.display.flip()
        self._clock.tick(60)
    ...
    def _render_panel(
        ...
        if self._screen is None:
            return
        panel_rect = pygame.Rect(video_width, 0, self.panel_width, height)
        ...
        self._button("toggle_motor", motor_label, pygame.Rect(x, y, 270, 46), motor_color)
        y += 54
        ...
        self._button("toggle_tracking", track_label, pygame.Rect(x, y, 270, 38), track_color)
        y += 54
        ...
        y += 34
        self._button("auto_target", "AUTO TARGET", pygame.Rect(x, y, 130, 38), (56, 72, 96))
        self._button("clear_target", "CLEAR LOCK", pygame.Rect(x + 140, y, 130, 38), (86, 56, 62))
        y += 58
        ...
        for track in sorted(tracks, key=_track_score, reverse=True)[:5]:
            ...
            self._button(f"target:{track_id}", label, pygame.Rect(x, y, 270, 32), color, font=self._small_font)
            y += 38
        ...
        status_y = min(max(y + 12, height - 104), height - 104)
        self._text(state.status, x, status_y, (255, 220, 120), self._small_font)
        self._text("M: motor   T: track   A: auto", x, height - 58, (150, 150, 160), self._small_font)
        self._text("C: clear   Q/Esc: quit", x, height - 34, (150, 150, 160), self._small_font)
    ...
    def _ensure_screen(self, video_width: int, video_height: int) -> None:
        size = (video_width + self.panel_width, video_height)
        if self._screen is None or size != self._window_size:
...
def main() -> None:
    args = parse_args()
    if args.enable_reid and args.require_cpp_tracker:
        raise SystemExit("--enable-reid is incompatible with --require-cpp-tracker because C++ tracker bindings do not accept embeddings")
    tracker_max_age = (
    ...
    if detector.device.startswith("cuda"):
        ui.render_status(ui_state, "Warming CUDA detector")
    ...
    print(f"[TRACKER] max_age={tracker_max_age} min_hits={args.track_min_hits}")
    ...
    def process(decoded: DecodedFrame, detections: np.ndarray, detection_age_s: float) -> None:
        ...
        if reid_helper is not None and detections.size:
            embeddings = reid_helper.build_embeddings(decoded.frame, detections, target_bbox)
            ui_state.reid_feature_count = sum(1 for feature in embeddings or [] if feature is not None)
        elif reid_helper is not None:
            ui_state.reid_feature_count = 0

        all_tracks = tracker.update(detections, decoded.frame.shape, embeddings=embeddings)
        now = time.monotonic()
        ...
        if target_track is None:
            ...
            error = compute_error(target_track, decoded.frame.shape)
            pan = p_control(error[0], args.kp_pan, args.deadband, args.max_speed)
            tilt = p_control(error[1], args.kp_tilt, args.deadband, args.max_speed)
